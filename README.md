...
**31 projects** · Last updated 2026-08-07
...
#### project-tracking
Cross-references funding PDFs, labor PDFs, and Microsoft Planner data to produce per-job dashboards (budget, costs, labor hours, submittal pipeline). React SPA (v2 UI) is now the primary interface. A job's data is built one of two ways: the classic pipeline (funding PDF + labor PDF + Planner xlsx → normalized report), or — for reviewer/admin users with a Sage selection — **Sage-primary** rows read live from the **Sage 100 Contractor Cloud Connector** (cost, budget, hours, commitments, AR/AP, contract value). The report fallback has been removed from that path, so Sage is now the only source for Sage-selected users. 2026 additions: Microsoft Graph Planner sync (no manual exports), SharePoint document sourcing, per-job completion-source picker, per-phase/CLIN breakdown, per-job ACLs + sharing, a portfolio AR/AP cash-position card with aging bars, and (August) **invoice-level AR/AP detail** plus a commitments-table flag for POs where Received exceeds Ordered. The EAC worksheet is built but currently hidden behind a `SHOW_EAC_WORKSHEET = false` frontend flag pending further work. **Stack:** Python (FastAPI, pdfplumber, openpyxl, mpxj), React 18 (Vite + TypeScript + Tailwind), account-store auth. Port 443 (self-terminated TLS, no proxy). ~610 tests across 85 files.
