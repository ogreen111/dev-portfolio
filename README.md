...
**31 projects** · Last updated 2026-08-08
...
#### rfp-automation
Automated extraction, analysis, and proposal generation for DoD/MILCON cybersecurity RFPs. Parses specs and drawings, classifies cyber governance (UFGS 25 05 11 / 25 08 11), extracts scope, builds submittal matrices, and generates draft proposals. 2026 additions: email intake with PIID identity gate, bid follow-up tracking with Planner comment integration, amendment cyber review (local Ollama for CUI, cloud otherwise), redesigned dashboard with SSE live updates, per-project Ask-AI chat, a launchd-driven SENTRY self-improvement loop, and (August) a **missing-RFP-docs flag** — a pipeline-independent watch-loop sweep that surfaces MISC-type project folders whose `0.RFP/` holds no source documents, which `parser.discover_projects` had been filtering out silently. It tracks each doc-less folder in a locked sidecar keyed by resolved absolute path, waits out a grace period before flagging, sends one Telegram alert at 24h plus one 7-day escalation and never repeats, and offers an operator acknowledge action; MRR/UMCS folders are never scanned. Surfaces in the dashboard attention inbox and the daily digest, behind `RFP_MISSING_DOCS_ENABLED`. **Stack:** Python (PyMuPDF, python-docx, openpyxl, Playwright, Claude API), FastAPI dashboard (port 8008). ~4,210 tests across 237 files — the largest suite in the portfolio.
...
#### project-tracking
Per-job dashboards (budget, costs, labor hours, submittal pipeline) for DoD construction-cybersecurity jobs, served as a React SPA (v2 UI). **Ingestion went Sage-only on 2026-08-07** (Phase 3 final cutover): reviewer/admin users pick jobs on the Sage Jobs page, and every tracked job's cost, budget, hours, commitments, AR/AP, and contract value are read live from the **Sage 100 Contractor Cloud Connector**. The funding-PDF / labor-PDF / normalized-report pipeline is gone — `excel_loader.py`, the funding/labor/job-status/WIP/phases/takeoffs parsers, and the report-generation subprocess were all deleted, along with their upload categories and SharePoint sync paths. Microsoft Planner sync survives as an optional layer contributing % Complete and bucket counts; since Sage selection is reviewer/admin-only, a `viewer` account can no longer track jobs of its own — it still sees jobs others share with it, since the shared-jobs merge isn't gated on the recipient's role. 2026 additions: SharePoint document sourcing, per-job completion-source picker, per-phase/CLIN breakdown, per-job ACLs + sharing, MS Project `.mpp` schedules via MPXJ, a portfolio AR/AP cash-position card with aging bars, invoice-level AR/AP detail, and (August) a broadened weekly **Executing Review** — scope predicate excluding Complete/Resources/MISC and completed cards, a permanent `planner_bucket_entries` table backing confirmed days-sitting, and an `execution_review_cache` with atomic claim/write guards for hourly sync. The EAC worksheet is built but currently hidden behind a `SHOW_EAC_WORKSHEET = false` frontend flag pending further work. **Stack:** Python (FastAPI, openpyxl, mpxj, Microsoft Graph), React 18 (Vite + TypeScript + Tailwind), account-store auth. Port 443 (self-terminated TLS, no proxy). ~584 tests across 79 files.
...
#### ssi-design-system
Single source of truth for SSi brand tokens (colors, typography, spacing, radii, shadows). CSS custom properties, Python brand.py, build + sync scripts. Now also home to the Niagara 4 engineering standard: point naming contract, point dictionary (xlsx), data-driven plant graphics rendered to PNG/Px/SVG, 10 standard Px view templates, kitPx artwork catalog, and a Px authoring guide. Also ships `interactions.js` — a small UMD module whose `guardAriaDisabled()` closes the keyboard-activation gap that `pointer-events: none` leaves open on `[aria-disabled="true"]` elements — now synced into all five on-disk consumers. **v0.1.0**, Phases 1–3 complete; project-tracking is the v0 pilot. 53 tests. `apps.json` marks six consumers enabled (project-tracking, rfp-automation, email-processor, cyber-artifact-gen, digital-twin, floor-plan-editor). The post-migration `sync.py` breakage is **fixed** (2026-08-06): `apps.json`'s `_root` now points at `~/dev`, and five of the six enabled targets resolve — `floor-plan-editor` is the lone miss, because that project has no directory anywhere (see its entry below).
...
ssi-design-system ←─── project-tracking (v0 pilot), rfp-automation, email-processor,
                       ...
                       (apps.json _root fixed 2026-08-06; 5 of 6 resolve —
                        floor-plan-editor has no directory to sync to)
